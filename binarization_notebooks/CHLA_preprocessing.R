...
get_triage_vitals_table <- function(data_RR, data_HR, data_So2, data_temp, data_blood_pressure, triage_vitals_times) {
  ...
  table_triage_vitals <- all_vitals %>%
    ...
    dplyr::select(encounter_id, vital_type, vital_value) %>%
    pivot_wider(
      names_from = vital_type,
      values_from = c(vital_value),
      names_sep = "_"
    ) %>%
...
get_all_vitals_table <- function(data_RR, data_HR, data_So2, data_temp, data_blood_pressure) {
  ...
  # Create wide table with one row per time stamp and one column per vital
  table_all_vitals <- all_vitals %>%
    pivot_wider(
      names_from = vital_type,
      values_from = vital_value,
      values_fn = first  # In case there are duplicates at the same timestamp
    ) %>%
    arrange(encounter_id, vitals_datetime)
  
  return(table_all_vitals)
}

# Add pain scores to vitals
add_pain_scores_to_vitals <- function(data_painscore, triage_vitals_wide, all_vitals_wide) {
  ...
  # Add pain scores to all vitals table
  all_vitals_with_pain <- all_vitals_wide %>%
    left_join(data_pain_processed, by = c("encounter_id", "vitals_datetime"))
...
get_temporal_variables <- function(data) {
  ...
  data <- data %>%
    mutate(
      # Extract year
      arrival_year = year(data$arrival_datetime),
      
      # Create season based on month
      arrival_season = case_when(
        month(data$arrival_datetime) %in% c(12, 1, 2) ~ "Winter (Dec-Feb)",
        month(data$arrival_datetime) %in% c(3, 4, 5) ~ "Spring (Mar-May)",
        month(data$arrival_datetime) %in% c(6, 7, 8) ~ "Summer (Jun-Aug)",
        month(data$arrival_datetime) %in% c(9, 10, 11) ~ "Fall (Sep-Nov)",
        TRUE ~ NA_character_
      ),
      
      arrival_day_type = case_when(
        wday(data$arrival_datetime) %in% c(6, 7) ~ "Weekend",
        wday(data$arrival_datetime) %in% c(1, 2, 3, 4, 5) ~ "Weekday",
        TRUE ~ NA_character_
      ),
      
      # Create 6-hour time blocks
      arrival_time_block = case_when(
        hour(data$arrival_datetime) %in% 0:5 ~ "00:00-05:59",
        hour(data$arrival_datetime) %in% 6:11 ~ "06:00-11:59",
        hour(data$arrival_datetime) %in% 12:17 ~ "12:00-17:59",
        hour(data$arrival_datetime) %in% 18:23 ~ "18:00-23:59",
        TRUE ~ NA_character_
      )
    )
...
get_crowdedness <- function(data) {
  data <- data %>%
    arrange(data$arrival_datetime) %>%
    mutate(
      crowdedness = purrr::map_dbl(row_number(), function(i) {
        current_arrival <- data$arrival_datetime[i]
        
        sum(data$arrival_datetime < current_arrival &
              disposition_datetime > current_arrival)
      })
    )
  return(data)
}
...
get_prior_visits <- function(data) { 
  visits <- data %>%
    arrange(data$patient_id, data$minutes_since_first_arrival) %>% 
    group_by(data$patient_id) %>%
    mutate(
      num_previous_admissions = purrr::map_dbl(row_number(), function(i) {
        current_time <- data$minutes_since_first_arrival[i]
        thirty_days_ago <- current_time - 24*60*30
        
        sum(data$minutes_since_first_arrival < current_time &
              data$minutes_since_first_arrival > thirty_days_ago &
...
get_demographics <- function(data, data_age, data_visit, data_weight, data_disposition,
  ...
  data$disposition <- group_disposition(data$disposition_raw)
  # Drop disposition = "Left" (42,069) ,"Died" (20) & "other/unknown" (272)
  data <- data %>%
    filter(!disposition %in% c("Left", "Died", "other/unknown"))
  
  # DISPOSITION TIME -----------------------------------------------------------
  data$disposition_datetime <- paste(data$pecarn_submit_eddisposition.eddepartdate,
                                     data$pecarn_submit_eddisposition.eddeparttime)
  data$disposition_datetime <- ymd_hms(data$disposition_datetime)
  ...
  data <- merge(data, data_visit,  
                ...
                all.x = TRUE)
  data$arrival_datetime <- paste(data$pecarn_submit_visitinformation.eddoordate,
                                 data$pecarn_submit_visitinformation.eddoortime)
  data$arrival_datetime <- ymd_hms(data$arrival_datetime)
  ...
  data <- data %>%
    mutate(
      minutes_since_first_arrival = as.numeric(difftime(data$arrival_datetime, min(data$arrival_datetime, na.rm = TRUE), units = "mins"))
    )
  
  # TRIAGE DATE & TIME ---------------------------------------------------------
  data$triage_datetime <- paste(data$pecarn_submit_visitinformation.triagedate,
                                data$pecarn_submit_visitinformation.triagetime)
  data$triage_datetime <- ymd_hms(data$triage_datetime)
  ...
  data$race_groups <- group_race(data$race)
  ...
  data$race_ethnicity <- group_race_ethnicity(data$race_groups, data$ethnicity)
  ...
  data$miles_travelled <- as.numeric(zip_distance(data$zip, "90027")$distance)
  ...
  for (complaint in names(perc_complaints)) {
    ...
    for (tag in tags) {
      ...
      column <- ifelse(match | column == 1, 1, 0)
    }
  ...
  data$insurance <- group_insurance(data$raw_insurance)
  ...
  data$arrival_mode <- group_arrival_mode(data$raw_arrival_mode)
  ...
  data$preferred_language <- group_preferred_language(data$raw_preferred_language)
  
  # CROWDEDNESS ----------------------------------------------------------------
  data$crowdedness <- get_crowdedness(data)
  ...
  data_weight$datetime <- paste (data_weight$date, data_weight$time)
  data_weight$datetime <- ymd_hms(data_weight$datetime)
  ...
  data_med_filtered <- data_meds %>%
    mutate(
      med_order_datetime = paste(`pecarn_submit_med_administration.medadministeredevent_date`,
                                 `pecarn_submit_med_administration.medadministeredevent_time`),
      med_order_datetime = ymd_hms(med_order_datetime)
    ) %>%
    left_join(
      data %>% dplyr::select(id_visit, arrival_datetime, disposition_datetime), 
  ...
  # IMAGING --------------------------------------------------------------------
  # ~ 600k entries without encounter ID, and the existent IDs do not match data
  ...
  data <- data[, c("id_visit", "id_patient", "disposition", "age", "age_group", 
                   ...
                   "any_IV_meds", "triage_vitals_time", names(diagnoses_indicators)[2:25],
                   "crowdedness", "num_previous_admissions", "num_previous_visits_without_admission",
                   "raw_complaint", "raw_reason_for_visit", names(complaint_groups))]
