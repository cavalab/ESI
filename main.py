...
from src.high_risk_dictionary import (
    ...
    cross_check_site_specific_keywords
)
...
def main(
        path_base='data',
        center: str = "BIDMC",  # Public release supports 'BIDMC' and 'Stanford'.
    ...
    input_path = os.path.abspath(os.path.expanduser(os.path.join(path_base, bin_data)))

    print(f'Working directory: {os.getcwd()}')
    print(f'Analysis input: {input_path}')
    print(f'Analysis output: {save_dir}')
    ...
    complaint_with_mask = cross_check_site_specific_keywords(
        complaint_with_mask, data_acuity, center, complaint_col
    )
    ...
    # Save CSV file
    assert 'ed_los' in complaint_all.columns, "ed_los column missing in complaint_all"
