...
simple_keywords = [
    ...
    'wheezing',
    # # WGL additions
    "upper respiratory infection", # congestion
    "upper respiratory tract infection", # congestion
    "substance ingestion", # overdose/poisoning
    "other: ingestion", # overdose/poisoning
    "aloc/behavior changes", # altered mental status
]
...
def cross_check_site_specific_keywords(complaint_with_mask, data_acuity, center, complaint_col):
    site_specific_keywords = {
        'BIDMC': ['sob'],
    }
    high_risk_complaint = pd.Series(False, index=data_acuity.index)
    if center in site_specific_keywords:
        for keyword in site_specific_keywords[center]:
            high_risk_complaint = (
                high_risk_complaint
                | data_acuity[complaint_col].str.contains(keyword, case=False, na=False)
            )
    updated_final_mask_corrected = ( 
        (complaint_with_mask['final_mask_corrected'])
        | (high_risk_complaint) 
    )
    print('site specific keyword updates to final mask corrected:', round((updated_final_mask_corrected.sum() - complaint_with_mask['final_mask_corrected'].sum())/len(complaint_with_mask)*100,1), '% increase in high-risk complaint detection')
    complaint_with_mask['final_mask_corrected'] = updated_final_mask_corrected
    return complaint_with_mask

def cross_check_complaint_contains(complaint_with_mask, data_acuity):
    cross_ref_keywords = [
        # "assault",
        # "altered mental status",
        # "poisoning",
    ...
    high_risk_complaint = pd.Series(False, index=data_acuity.index)
    for complaint in [c for c in data_acuity.columns if c.startswith("complaint_contains_")]:
        if complaint in cross_ref_keywords:
            high_risk_complaint = (high_risk_complaint) | data_acuity[complaint]

    # high_risk_complaint = [sc in simple_keywords for i,sc in enumerate(short_complaints)]
    updated_final_mask_corrected = ( 
        (complaint_with_mask['final_mask_corrected'])
        | (high_risk_complaint) 
    )
    print('"complaint_contains" updates to final mask corrected:', round((updated_final_mask_corrected.sum() - complaint_with_mask['final_mask_corrected'].sum())/len(complaint_with_mask)*100,1), '% increase in high-risk complaint detection')
    complaint_with_mask['final_mask_corrected'] = updated_final_mask_corrected
    return complaint_with_mask
